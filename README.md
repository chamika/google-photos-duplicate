...
- `--exclude-album NAME` — Album folder whose photos are never marked for deletion; repeat for multiple albums, pass `''` to disable (default: `Instagram`). Protected photos are preferred as the group's "keep", and identical copies elsewhere (same content or same Google Photos URL) are protected too
